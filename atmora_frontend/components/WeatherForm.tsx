...
const WeatherForm: React.FC<WeatherFormProps> = ({ 
  ...
  const [formData, setFormData] = useState<WeatherAnalysisParams>({
    ...
    thresholds: {
      temperature: 35, // Very hot threshold (°C)
      windSpeed: 15, // Very windy threshold (m/s)
      precipitation: 20, // Very wet threshold (mm)
      humidity: 80 // Very humid threshold (%)
    }
  });
        ...
        <div className="overflow-y-auto max-h-[calc(80vh-80px)]">
          <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Thresholds */}
              <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Weather Condition Thresholds</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <input
                      type="number"
                      value={formData.thresholds.temperature}
                      onChange={(e) => setFormData({
                        ...formData, 
                        thresholds: {...formData.thresholds, temperature: parseFloat(e.target.value)}
                      })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Very Windy Speed (m/s)
                    </label>
                    <input
                      type="number"
                      value={formData.thresholds.windSpeed}
                      onChange={(e) => setFormData({
                        ...formData, 
                        thresholds: {...formData.thresholds, windSpeed: parseFloat(e.target.value)}
                      })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({
                        ...formData, 
                        thresholds: {...formData.thresholds, precipitation: parseFloat(e.target.value)}
                      })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={formData.thresholds.humidity}
                      onChange={(e) => setFormData({
                        ...formData, 
                        thresholds: {...formData.thresholds, humidity: parseFloat(e.target.value)}
                      })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    />
                  </div>
